...
class SequenceGenerator:
    ...
    def generate_beam(
        ...
        batch_size = x.size(0)
        initial_scores = self.score_fn(x)
        initial_scores = self._apply_repeat_penalty(initial_scores, x, repeat_penalty)
        initial_scores = initial_scores / temperature
        initial_log_probs = torch.log_softmax(initial_scores, dim=-1)

        scores, next_tokens = torch.topk(initial_log_probs, k=beam_width, dim=-1)
        ...
        for _ in range(self.max_length - sequences.size(-1)):
            ...
            gather_index = beam_indices.unsqueeze(-1).expand(-1, -1, sequences.size(-1))
            sequences = torch.gather(sequences, 1, gather_index)
            sequences = torch.cat([sequences, token_indices.unsqueeze(-1)], dim=-1)
            finished = torch.gather(finished, 1, beam_indices) | (token_indices == self.tokenizer.eos_id)
