...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        autocast_dtype = torch.float16 if self.device.startswith("cuda") else torch.bfloat16
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type='cuda' if self.device.startswith("cuda") else 'cpu', dtype=autocast_dtype):
    ...
    def _validate_epoch(self, dataloader):
        ...
        results = self.recognize(
            dataloader,
            recognition_config={'num_batches': None, 'beam_width': 1, 'temperature': 1.0, 'repeat_penalty': 1.0, 'lm_weight': 0.0, 'lm_model': None},
            config_name='greedy'
        )

        if not results or 'target' not in results[0]:
            return {'word_dist': 0.0, 'wer': 0.0, 'cer': 0.0}, results

        references = [item['target'] for item in results]
        hypotheses = [item['generated'] for item in results]
        return self._calculate_asr_metrics(references, hypotheses), results
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                prompts = torch.full(
                    (batch_size, 1), self.tokenizer.sos_id, dtype=torch.long, device=self.device
                )
